# Small Application Feature Prompts

<!--
Purpose: This file is the reusable scenario pool for benchmark runs. Each
numbered prompt should be agent-agnostic, product-focused, and stable enough to
replay across a frozen baseline and candidate optimization passes.
-->

1. Create a simple to-do list app where I can add tasks, mark tasks as complete, edit task names, and delete tasks I no longer need. The app should show active and completed tasks separately.

3. Create a small app that helps me manage books I want to read, books I am reading, and books I have finished. I should be able to rate finished books, add short notes, and filter the list by reading status.

4. I need a meal planning app where I can plan meals for each day of the week and keep a reusable list of recipes. The app should let me assign recipes to days and generate a shopping list from the planned meals.

5. Create a dashboard that lets me track maintenance tasks for personal items such as a car, laptop, bicycle, or appliance. Each item should support recurring tasks, due dates, completion history, and notes.

6. I need a small app to track job applications. It should let me add companies, roles, application dates, current status, contacts, notes, and reminders for follow-up actions.
